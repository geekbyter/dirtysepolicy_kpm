/*
 * DirtySepolicy/DuckDetector SELinux query filter for KernelPatch KPM.
 *
 * Forked from Admirepowered/selinux_hook (GPLv3).  The original module
 * targeted Magisk policy probes; this variant keeps the clean-policy
 * redirection path and broadens the fallback string filters for
 * DirtySepolicy/DuckDetector root-framework contexts.
 *
...
static bool dirtyduck_should_mask_access_query(const char *query);
static bool dirtyduck_should_block_context_query(const char *query, size_t len);
static ssize_t patch_response_allowed_zero(char *buf, ssize_t ret);
...
/*
 * Patch the first token (allowed bitmask) in a selinuxfs access response.
 * Format: "%x %x %x %x %u %x"
 */
static ssize_t patch_response_allowed_zero(char *buf, ssize_t ret)
{
    ...
    p[0] = '0';
    move = (size_t)(end - tok_end);
    if (tok_end != p + 1) {
        for (j = 0; j < (int)move; j++)
            p[1 + j] = tok_end[j];
        ret -= (ssize_t)(tok_end - (p + 1));
    }
...
{
    /*
     * Android 4.14 can reach security_compute_av() from sel_write_context()
...
static bool legacy_should_block_access_query(const char *query, size_t len)
{
    ...
    if (contains_case_literal(query, len, "magisk"))
        return true;
    if (contains_case_literal(query, len, "kitsune"))
        return true;
    if (contains_case_literal(query, len, "apatch"))
        return true;
    if (contains_case_literal(query, len, "ksu"))
        return true;
    if (contains_case_literal(query, len, "kernelsu"))
        return true;
    if (contains_case_literal(query, len, "ksu_file"))
        return true;
    if (contains_case_literal(query, len, "lsposed"))
        return true;
    if (contains_case_literal(query, len, "lsposed_file"))
        return true;
    if (contains_case_literal(query, len, "xposed"))
        return true;
    if (contains_case_literal(query, len, "xposed_data"))
        return true;
    if (contains_case_literal(query, len, "riru"))
        return true;
    if (contains_case_literal(query, len, "adbroot"))
        return true;
    if (contains_case_literal(query, len, "supersu"))
        return true;
    if (contains_case_literal(query, len, "supolicy"))
        return true;
    if (contains_case_literal(query, len, "zygisk"))
        return true;
    if (contains_case_literal(query, len, "droidspacesd"))
        return true;
    if (contains_case_literal(query, len, "msd_daemon"))
        return true;
    if (contains_case_literal(query, len, "msd_app"))
        return true;
...
{
    if (!query || !len || !scon || !tcon)
        return false;
    return contains_case_lit(query, len, scon, token_len(scon)) &&
           contains_case_lit(query, len, tcon, token_len(tcon));
}

static bool dirtyduck_should_block_context_query(const char *query, size_t len)
{
    return legacy_should_block_access_query(query, len);
}
...
static bool dirtyduck_should_mask_access_query(const char *query)
{
    ...
    for (len = 0; len < ACCESS_SAMPLE_MAX - 1 && query[len]; len++) {
    }

    if (legacy_should_block_access_query(query, len))
        return true;

    /* DirtySepolicy stock-context probes.  These contexts are legitimate
     * Android labels, so context-existence must still pass; only their
     * queried allow bits should be hidden from app UID probes.
     */
    if (query_has_pair(query, len, "u:object_r:rootfs:s0", "u:object_r:tmpfs:s0"))
        return true;
    if (query_has_pair(query, len, "u:r:kernel:s0", "u:object_r:tmpfs:s0"))
        return true;
    if (query_has_pair(query, len, "u:r:kernel:s0", "u:object_r:adb_data_file:s0"))
        return true;
    if (query_has_pair(query, len, "u:r:system_server:s0", "u:object_r:apk_data_file:s0"))
        return true;
    if (query_has_pair(query, len, "u:r:dex2oat:s0", "u:object_r:dex2oat_exec:s0"))
        return true;
    if (query_has_pair(query, len, "u:r:zygote:s0", "u:object_r:adb_data_file:s0"))
        return true;

    /* DuckDetector additions from duckdetector_analysis.md. */
    if (query_has_pair(query, len, "u:r:shell:s0", "u:r:su:s0"))
        return true;
    if (query_has_pair(query, len, "u:r:system_server:s0", "u:r:system_server:s0"))
        return true;
    if (query_has_pair(query, len, "u:r:fsck:s0", "u:r:fsck:s0"))
        return true;
    if (query_has_pair(query, len, "u:r:fsck_untrusted:s0", "u:r:fsck_untrusted:s0"))
        return true;
...
static void before_sel_write_context(hook_fargs4_t *a, void *u)
{
    ...
    if (dirtyduck_should_block_context_query(sample, sample_len)) {
        n = READ_ONCE(g_clean_access_count) + 1;
        WRITE_ONCE(g_clean_access_count, n);
        a->local.data0 = 2;
        a->local.data1 = n;
...
static void after_sel_write_common(hook_fargs4_t *a, void *u)
{
    ...
    if (live_ret > 0 && probe->node && probe->node[0] == 'a') {
        ...
        if (dirtyduck_should_mask_access_query(probe->query)) {
            ssize_t masked_ret = patch_response_allowed_zero(rbuf, live_ret);
            if (masked_ret > 0) {
                live_ret = masked_ret;
                a->ret = (uint64_t)masked_ret;
            }
            selinux_hook_dbg("[selinux_hook] DIRTYDUCK masked access uid=%d comm=%s query=\"%s\"\n",
                             probe->uid, current_comm(), probe->query);
        }
...
static long init(const char *args, const char *event, void *__user r)
{
    ...
    copy_to_user_nofault_fn = (void *)kallsyms_lookup_name("copy_to_user_nofault");
    if (copy_to_user_nofault_fn) {
        g_copy_to_user_name = "copy_to_user_nofault";
    } else {
        copy_to_user_raw_fn = (void *)kallsyms_lookup_name("_copy_to_user");
        if (copy_to_user_raw_fn) {
            g_copy_to_user_name = "_copy_to_user";
        } else {
            copy_to_user_raw_fn = (void *)kallsyms_lookup_name("__copy_to_user");
            if (copy_to_user_raw_fn)
    ...
    vmalloc_fn = (void *)kallsyms_lookup_name("vmalloc");
    if (!vmalloc_fn)
    ...
    security_load_policy_fn = (void *)kallsyms_lookup_name("security_load_policy");
    security_load_policy_compat_fn = (void *)security_load_policy_fn;
    security_context_to_sid_fn = (void *)kallsyms_lookup_name("security_context_to_sid");
    security_context_to_sid_compat_fn = (void *)security_context_to_sid_fn;
    policydb_read_fn = (void *)kallsyms_lookup_name("policydb_read");
    policydb_destroy_fn = (void *)kallsyms_lookup_name("policydb_destroy");
    flex_array_get_fn = (void *)kallsyms_lookup_name("flex_array_get");
    avtab_search_node_fn = (void *)kallsyms_lookup_name("avtab_search_node");
    ...
    security_read_policy_fn = (void *)kallsyms_lookup_name("security_read_policy");
    ...
    addr = (unsigned long)kallsyms_lookup_name("simple_read_from_buffer");
    if (addr) {
    ...
    addr = (unsigned long)kallsyms_lookup_name("string_to_context_struct");
    ...
    addr = (unsigned long)kallsyms_lookup_name("selinux_complete_init");
    if (addr) {
        g_funcs[g_hooks++] = (void *)addr;
    ...
    addr = (unsigned long)kallsyms_lookup_name("selinux_policy_commit");
    if (addr) {
        g_funcs[g_hooks++] = (void *)addr;
